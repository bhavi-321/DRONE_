...
class DroneController:
    ...
    def arm_and_takeoff(self, target_altitude, cap=None):
        ...
        while not self.vehicle.is_armable:
            ...
            self.wait_and_stream(1, cap) # Stream while waiting
        ...
        while True:
            ...
            # Only log every 1 second roughly
    ...
    def land_and_rtl(self, home):
        ...
        while self.vehicle.location.global_relative_frame.alt > 3:
            self.send_local_ned_velocity(0, 0, 1, 1)
        ...
        try:
            ...
            if camera_type == "Raspberry Pi Camera":  # RPi Camera
                cap = cv2.VideoCapture(
                    "libcamerasrc ! video/x-raw,width=640,height=480,framerate=10/1 ! videoconvert ! appsink drop=1 ",
                    cv2.CAP_GSTREAMER)
                # cap = cv2.VideoCapture(0)
    ...
    def center_above_qr_region_based(self, cap):
        ...
        while True:
            ...
            if qrs:
                ...
                # Check if centered
                if region == "CENTERED" and self.vehicle.location.global_relative_frame.alt < 3:
                ...
                vy = -vy
                self.drone.send_local_ned_velocity(vy/2, vx/2, 0.1, 1)
...
class DroneControlGUI:
    ...
    def start_camera_preview(self):
        ...
        def init_cam_thread():
            ...
            # You can change this string to "Laptop Camera" for testing
    ...
    def start_camera_loop(self):
        ...
        def camera_loop():
            ...
            while self.preview_active:  # Check flag
                # If we have a valid camera, read from it
                if self.cap and self.cap.isOpened():
                    ...
                    time.sleep(0.5)
                # Cap frame rate to ~30fps to save CPU
                time.sleep(0.03) 
    ...
    def create_main_panel(self):
        ...
        # self.conn_type_var = tk.StringVar(value="Real Drone (USB)")
        ...
        try:
            ...
            while True:
                ...
                if msg_type == "camera_frame":
                    # REASON #2 FIX: Don't update GUI yet! Just save the data.
                    # This overwrites old frames, effectively discarding them.
                    last_frame = msg_data 
        ...
        def mission_thread():
            ...
            try:
                ...
                if cap is None:
                    ...
                    # Important: Exit the thread here because the blind mission 
                    # handles the landing and cleanup itself.
                    with self.mission_lock:
                        self.mission_running = False
